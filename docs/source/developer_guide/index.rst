...
``aiida-fujitsu-scheduler`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-fujitsu-scheduler`` repository (preferably using ``aiida-fujitsu-scheduler`` as the project name)

The documentation is now available at `aiida-fujitsu-scheduler.readthedocs.io <http://aiida-fujitsu-scheduler.readthedocs.io/>`_.
